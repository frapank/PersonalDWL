...
static dbus_bool_t dwl_dbus_add_watch(DBusWatch* watch, void* data)
{
    ...
    watch_source =
        wl_event_loop_add_fd(loop, fd, mask, dwl_dbus_watch_handle, watch);
...
static void dwl_dbus_dispatch_status(DBusConnection* conn,
                                 ...
                                 struct wl_event_loop* loop)
{
    ...
    bus_source = wl_event_loop_add_fd(
        loop, pipefd[0], WL_EVENT_READABLE, dwl_dbus_dispatch, conn);
    if (!bus_source)
        goto fail;

    dbus_connection_set_dispatch_status_function(
        conn, dwl_dbus_dispatch_status, pipefd, close_pipe);
