...
static DBusHandlerResult handle_nameowner_changed(Watcher* watcher,
                                                  DBusConnection* conn,
                                                  DBusMessage* msg)
{
...
static DBusHandlerResult filter_bus(DBusConnection* conn,
                                    DBusMessage* msg,
                                    void* data)
{
...
static DBusHandlerResult respond_register_item(Watcher* watcher,
                                               DBusConnection* conn,
                                               DBusMessage* msg)
{
    ...
    if (!(sender = dbus_message_get_sender(msg)) ||
        !dbus_message_get_args(
            msg, NULL, DBUS_TYPE_STRING, &param, DBUS_TYPE_INVALID)) {
        reply = dbus_message_new_error(
            msg, DBUS_ERROR_INVALID_ARGS, "Malformed message");
        goto send;
    }
    ...
    switch (*param) {
        ...
        case ':':
            ...
            break;
        default:
            /* KStatusNotifierItem registers under its well-known name
             * rather than a unique one or an object path */
            registree_name = param;
            busobj = SNI_OPATH;
            break;
    }
    ...
    if (!dbus_validate_bus_name(registree_name, NULL)) {
        reply = dbus_message_new_error_printf(
            msg, DBUS_ERROR_INVALID_ARGS, "Invalid busname %s", registree_name);
        goto send;
    }

    if (item_name_to_ptr(watcher, registree_name)) {
        reply = dbus_message_new_error_printf(
            msg, DBUS_ERROR_INVALID_ARGS, "%s already tracked", registree_name);
        goto send;
    }
...
static int get_registered_items(const Watcher* watcher, DBusMessageIter* iter)
{
    ...
    if (!dbus_message_iter_open_container(
            iter, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING_AS_STRING, &names)) {
        r = -ENOMEM;
        goto fail;
    }
    ...
    wl_list_for_each(item, &watcher->items, link)
    {
        if (!dbus_message_iter_append_basic(
                &names, DBUS_TYPE_STRING, &item->busname)) {
            r = -ENOMEM;
            goto fail;
        }
    }
...
static int get_registered_items_variant(const Watcher* watcher,
...
static int get_isregistered(DBusMessageIter* iter)
{
    ...
    if (!dbus_message_iter_open_container(
            iter, DBUS_TYPE_VARIANT, DBUS_TYPE_BOOLEAN_AS_STRING, &variant) ||
        !dbus_message_iter_append_basic(
            &variant, DBUS_TYPE_BOOLEAN, &is_registered)) {
        r = -ENOMEM;
        goto fail;
    }
...
static int get_version(DBusMessageIter* iter)
{
    ...
    if (!dbus_message_iter_open_container(
            iter, DBUS_TYPE_VARIANT, DBUS_TYPE_INT32_AS_STRING, &variant) ||
        !dbus_message_iter_append_basic(
            &variant, DBUS_TYPE_INT32, &protovers)) {
        r = -ENOMEM;
        goto fail;
    }
...
static DBusHandlerResult respond_get_prop(Watcher* watcher,
                                          DBusConnection* conn,
                                          DBusMessage* msg)
{
    ...
    if (strcmp(prop, "ProtocolVersion") == 0) {
        ...
        dbus_message_unref(reply);
        reply = dbus_message_new_error_printf(reply,
...
static DBusHandlerResult respond_all_props(Watcher* watcher,
                                           DBusConnection* conn,
                                           DBusMessage* msg)
{
    DBusMessage* reply = NULL;
    ...
    if (!dbus_message_iter_open_container(
            &iter, DBUS_TYPE_ARRAY, "{sv}", &array))
        goto fail;

    prop = "ProtocolVersion";
    if (!dbus_message_iter_open_container(
            &array, DBUS_TYPE_DICT_ENTRY, NULL, &dict) ||
...
static DBusHandlerResult respond_introspect(DBusConnection* conn,
                                            DBusMessage* msg)
{
    DBusMessage* reply = NULL;
    ...
    if (!dbus_message_append_args(
            reply, DBUS_TYPE_STRING, &snw_xml, DBUS_TYPE_INVALID) ||
        !dbus_connection_send(conn, reply, NULL)) {
        goto fail;
    }
...
static DBusHandlerResult snw_message_handler(DBusConnection* conn,
                                             DBusMessage* msg,
                                             void* data)
{
    Watcher* watcher = data;
    ...
    if (dbus_message_is_method_call(
            msg, DBUS_INTERFACE_INTROSPECTABLE, "Introspect"))
        return respond_introspect(conn, msg);

    else if (dbus_message_is_method_call(
                 msg, DBUS_INTERFACE_PROPERTIES, "GetAll"))
...
void watcher_start(Watcher* watcher,
                   DBusConnection* conn,
                   struct wl_event_loop* loop)
{
    ...
    if (!dbus_connection_register_object_path(
            conn, SNW_OPATH, &snw_vtable, watcher)) {
